...
#5,5 is the filter size, 3 is the number of input channels from previous layer and 32 is the number of output channels
    w_conv1 = assign_weights([5,5,3,32])
    b_conv1 = assign_bias([32])
    h_conv1 = tf.nn.relu(conv2d(input_image,w_conv1)+b_conv1)
    h_drop1 = tf.nn.dropout(h_conv1,dropout)
    h_pool1 = maxpool2d(h_conv1,k=4)

    #Second Layer:
#5,5 is the filter size, 32 is the number of input channels and 200 is the number of output channels
    w_conv2 = assign_weights([5,5,32,200])
    b_conv2 = assign_weights([200])
    h_conv2 = tf.nn.relu(conv2d(h_pool1,w_conv2)+b_conv2)
    h_drop2 = tf.nn.dropout(h_conv2,dropout)
    h_pool2 = maxpool2d(h_drop2,k=4)

    #Affine Layer
    w_fc1 = assign_weights([2*2*200,1000])
    b_fc1 = assign_bias([1000])
    h_pool2_flat = tf.reshape(h_pool2,[-1,2*2*200])
    ...
    return h_conv1,h_pool1,h_conv2,h_pool2,out
...
with tf.name_scope('model'):
    h_conv1,h_pool1,h_conv2,h_pool2,pred = conv_net(inputs,dropout)
...
for e in range(num_epoch):
    ...
    for b, (input_batch, target_batch) in enumerate(train_data):
        ...
        conv1,pool1,conv2,pool2,_, summary, batch_error, batch_acc = sess.run(
            [h_conv1,h_pool1,h_conv2,h_pool2,train_step, summary_op, error, accuracy],
...
np.savez_compressed(
    os.path.join(exp_dir, 'exp_run_cnn_dropout05_beforeMP_'+str(num_epoch)+'.npz'),
    train_error=train_error,
    train_accuracy=train_accuracy,
    valid_error=valid_error,
    valid_accuracy=valid_accuracy,
    conv1_layer=np.asarray(conv1_layer, dtype='float64'),
    pool1_layer=np.asarray(pool1_layer, dtype='float64'),
