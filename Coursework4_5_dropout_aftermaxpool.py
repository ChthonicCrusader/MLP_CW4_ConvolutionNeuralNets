...
def conv_net(x,dropout):
    ...
    h_drop1 = tf.nn.dropout(h_pool1,dropout)
    ...
    h_drop2 = tf.nn.dropout(h_pool2,dropout)

    #Affine Layer
    w_fc1 = assign_weights([2*2*200,1000])
    b_fc1 = assign_bias([1000])
    h_pool2_flat = tf.reshape(h_drop2,[-1,2*2*200])
...
np.savez_compressed(
    os.path.join(exp_dir, 'exp_run_cnn_dropout5_afterMP'+str(num_epoch)+'.npz'),
