...
def conv_net(x):
    ...
    #Affine Layer
    w_fc1 = assign_weights([2*2*200,1024])
    b_fc1 = assign_bias([1024])
    h_pool2_flat = tf.reshape(h_pool2,[-1,2*2*200])
    h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat,w_fc1)+b_fc1)


    #output layer
    w_fc2 = assign_weights([1024,10])
...
with tf.name_scope('model'):
    h_conv1,h_pool1,h_conv2,h_pool2,pred = conv_net(inputs)
...
np.savez_compressed(
    os.path.join(exp_dir, 'exp_run_cnn_graddescent_'+str(num_epoch)+'.npz'),
    ...
    pool2_layer=np.asarray(pool2, dtype='float64'),
