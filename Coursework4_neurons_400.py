...
def conv_net(x):
    ...
    #Second Layer:
    w_conv2 = assign_weights([5,5,32,400])
    b_conv2 = assign_weights([400])
    h_conv2 = tf.nn.relu(conv2d(h_pool1,w_conv2)+b_conv2)
    h_pool2 = maxpool2d(h_conv2,k=4)

    #Affine Layer
    w_fc1 = assign_weights([2*2*400,1000])
    b_fc1 = assign_bias([1000])
    h_pool2_flat = tf.reshape(h_pool2,[-1,2*2*400])
...
np.savez_compressed(
    os.path.join(exp_dir, 'exp_neurons400_cnn_'+str(num_epoch)+'.npz'),
